...
utility.dragify = function(ins,touse)
	...
	ins.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			-- Don't drag if any slider or colorpicker is being interacted with
...
function Clover:new(props)
    ...
    local outline = utility.new(
        "Frame",
        {
            ...
            BorderSizePixel = 1,
            Size = menuSize, -- Set size from props
    ...
    setmetatable(window, Clover)
    return window
end

--
function Clover:watermark()
	local watermark = {}
	--
	local outline = utility.new(
		"Frame",
...
--
function Clover:oldloader(props)
...
--
function Clover:loadconfig(cfg)
...
--
function Clover:settheme(theme,color)
	local window = self
	--
...
--
function Clover:settoggle(side,bool)
	if side == "x" then
		self.x = bool
	else
		self.y = bool
...
--
function Clover:settextsize(size)
	if size ~= nil then
		local window = self
		for i,v in pairs(window.labels) do
			if v ~= nil then
...
function sections:label(props)
	-- // properties
	local name = props.name or props.Name or props.text or props.Text or "new label"
	-- // variables
	local label = {}
	-- // main
	...
	--
	self.library.labels[#self.library.labels+1] = labeltext
	-- // return
	return label
end
--
...
--
function Clover:closewindows(ignore)
	local window = self
	--
...
function sections:colorpicker(props)
	...
	--
	uis.InputChanged:Connect(function(input)
		-- Support both mouse and touch for colorpicker
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
...
--
function Clover:loader(props)
	-- // properties
	local username = props.username or props.Username or props.user or props.User or plr.Name
	local loadTime = props.loadtime or props.LoadTime or props.time or props.Time or 2
	local callback = props.callback or props.callBack or props.CallBack or props.Callback or function()end
	-- // main
	local loaderscreen = utility.new(
		"ScreenGui",
		{
			Name = "CloverLoader_"..tostring(math.random(0, 999999)),
			DisplayOrder = 99999,
			ResetOnSpawn = false,
			ZIndexBehavior = "Global",
			Parent = cre
	...
	local boxHeight = isMobile and math.min(viewportSize.Y * 0.6, 250) or 300
	--
	local container = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundColor3 = Color3.fromRGB(225, 58, 81),
			BorderColor3 = Color3.fromRGB(12, 12, 12),
			BorderSizePixel = 1,
	...
	utility.new(
		"UISizeConstraint",
		{
			...
			MaxSize = Vector2.new(viewportSize.X * 0.9, viewportSize.Y * 0.8),
			Parent = container
		}
	)
	--
	local container2 = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundColor3 = Color3.fromRGB(0, 0, 0),
	...
	--
	local profileholder = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5, 0),
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	--
	local profileoutline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
			BorderColor3 = Color3.fromRGB(56, 56, 56),
	...
	--
	local profileimage = utility.new(
		"ImageLabel",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
			BorderSizePixel = 0,
			Size = UDim2.new(1, 0, 1, 0),
			Image = "rbxthumb://type=AvatarHeadShot&id="..plr.UserId.."&w=420&h=420",
			Parent = profileoutline
		}
	)
	--
	local welcometext = utility.new(
		"TextLabel",
		{
			AnchorPoint = Vector2.new(0.5, 0),
			BackgroundTransparency = 1,
	...
	local welcomemsg = utility.new(
		"TextLabel",
		{
			...
			Font = "RobotoMono",
			Text = "Welcome! Thanks for using Clover Productions.",
	...
	--
	local loadingbaroutline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
			BorderColor3 = Color3.fromRGB(56, 56, 56),
	...
	--
	-- Animate loading bar
	ts:Create(loadingfill, TweenInfo.new(loadTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0)}):Play()
	--
	wait(loadTime)
	callback()
	--
	...
	ts:Create(profileholder, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
	ts:Create(profileoutline, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
	ts:Create(profileimage, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1, BackgroundTransparency = 1}):Play()
	ts:Create(welcometext, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
	ts:Create(welcomemsg, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
	ts:Create(loadingbar, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
	ts:Create(loadingbaroutline, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
	ts:Create(loadingfill, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
	--
	wait(0.5)
